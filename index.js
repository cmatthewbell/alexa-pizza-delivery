...
const invocationName = "pizza delivery";
...
const OrderPizza_Handler = {
  canHandle(handlerInput) {
    const request = handlerInput.requestEnvelope.request;
    return (
      request.type === "IntentRequest" && request.intent.name === "OrderPizza"
    );
  },
  handle(handlerInput) {
    const request = handlerInput.requestEnvelope.request;
    ...
    function orderPizza() {
      let myStore = new pizzapi.Store(1175);
      myStore.ID = 1175;

      let chris = new pizzapi.Customer({
        firstName: "chris",
        lastName: "bell"
      });
    ...
    let say = "Okay pizza has been ordered.";
...
const LaunchRequest_Handler = {
  ...
  handle(handlerInput) {
    ...
    let say =
      "Welcome to pizza delivery. You can say order a pizza, or order pizza, to order a pizza for carryout.";
    ...
    languageModel: {
      ...
      intents: [
        ...
        },
        {
          name: "OrderPizza",
          slots: [],
          samples: ["order pizza", "order my pizza", "order a pizza"]
        },
        {
          name: "LaunchRequest"
        }
